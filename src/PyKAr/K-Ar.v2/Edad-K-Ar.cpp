/*
Rutinas de la clase TKArData para calcular la edad por el metodo de K-Ar.
la rutina de edad Age() fue adaptada del programa nedad.c,
desarrollado por la Dra. M. Lopez.
...
__fastcall TKArData::TKArData(void)
{
	strcpy(InFile,"\0");
	strcpy(OutFile,"\0");
   	DataOk= FALSE;
...
__fastcall TKArData::TKArData(char *NameIn, char *NameOut)
{
	strcpy(InFile,NameIn);
	strcpy(OutFile,NameOut);
    ReadFile();
}
//Poner los datos del archivo Name en memoria
__fastcall TKArData::ReadFile(char *Name)
{
	strcpy(InFile,Name);
    ReadFile();
    return 0;
}
//Poner los datos de un archivo DAT en memoria
__fastcall TKArData::ReadFile(void)
{
	char buff[0x200];
	FILE *in;
	...
	for (int i = 0; i<nb; i++)
    {
		...
		sscanf(buff, "%f%f%f%f%f%f%f%f%f%f%f%f%f",
        	&tb36[i],&b[0][i],&b[1][i],&b[2][i],&b[3][i],&b[4][i],&b[5][i],
            &b[6][i],&b[7][i],&b[8][i],&b[9][i],&b[10][i],&b[11][i]);
	}
	...
	for (int i = 0; i<n; i++)
    {
    	fgets(buff,sizeof buff,in);
		sscanf(buff,"%f%f%f%f%f%f%f%f%f%f%f%f%f",
        	&tx36[i],&x[0][i],&x[1][i],&x[2][i],&x[3][i],&x[4][i],&x[5][i],
            &x[6][i],&x[7][i],&x[8][i],&x[9][i],&x[10][i],&x[11][i]);
	}
   	fgets(buff,sizeof buff,in);
	sscanf(buff,"%f%f%f%f", &sp4038, &sgsp4038, &sp3638, &sgsp3638);
   	fgets(buff,sizeof buff,in);
	sscanf(buff,"%f%f%f", &az36, &az38, &az40);
   	fgets(buff,sizeof buff,in);
	sscanf(buff,"%f%f%f%f", &tcero, &sgtcero, &delta, &sgdelta);
   	fgets(buff,sizeof buff,in);
	sscanf(buff,"%s%[^\n]", &AtmName, &AtmDate);
   	fgets(buff,sizeof buff,in);
	sscanf(buff,"%f%f", &atm4036, &sgatm4036);
   	fgets(buff,sizeof buff,in);
	sscanf(buff,"%s", &SpkName);
   	fgets(buff,sizeof buff,in);
	sscanf(buff,"%f%f%f%f", &spikeno, &peso, &pot, &sgpot);
	fclose(in);
    DataOk= TRUE;
    return 0;
}

__fastcall TKArData::Age(void)
{
    ...
    float s36 = 0.0, sx36 = 0.0;
	float s38 = 0.0, sx38 = 0.0;
	float s40 = 0.0, sx40 = 0.0;
	float sb36 = 0.0, sz36 = 0.0;
	float sb38 = 0.0, sz38 = 0.0;
	float sb40 = 0.0, sz40 = 0.0;
	...
	float eq6, eq8, eq0, seq6 = 0.0, seq8 = 0.0, seq0 = 0.0;
	...
	float m4038 = 0.0, sgm4038 = 0.0, m3836 = 0.0, sgm3836 = 0.0;
	// cocientes medidos en la muestra y su error
	float esp = 0.0;
	float sp38 = 0.0, sgsp38 = 0.0, sqs38 = 0.0, sqsp38 = 0.0;
	...
	float tot40 = 0.0, vol40 = 0.0, rad40 = 0.0;
	// vol40 es ccSTP por gramo, atomar es atomos de 40Ar por gramo
	float pot40 = 0.0 , sgpot40 = 0.0;
	...
	for (int i = 0; i<nb; i++)
    {
		a36 = b[1][i] - (b[0][i] + b[2][i]) / 2.0;
		a38 = b[5][i] - (b[4][i] + b[6][i]) / 2.0;
		a40 = b[9][i] - (b[8][i] + b[10][i]) / 2.0;
	...
	sbm36 = sqrt (( ss36c / den36 ) * ( sub36 / (nb - 2 )));
	sgl36 = sqrt (( sv36 / den36 ) * ( sub36 / (nb - 2)));
	sbm38 = sqrt (( ss38c / den38 ) * ( sub38 / (nb -2 )));
	sgl38 = sqrt (( sv38 / den38 ) * ( sub38 / (nb - 2)));
	sbm40 = sqrt (( ss40c / den40 ) * ( sub40 / (nb - 2)));
	sgl40 = sqrt (( sv40 / den40 ) * ( sub40 / (nb - 2)));
	...
	fprintf(out,"\n");//escribir los parametros de la recta
	...
	for( int i = 0; i<n; i++ )
    {
		y36 = x[1][i] - (x[0][i] + x[2][i]) / 2.0;
		y38 = x[5][i] - (x[4][i] + x[6][i]) / 2.0;
		y40 = x[9][i] - (x[8][i] + x[10][i]) / 2.0;
		p36[i] = y36;
		p38[i] = y38;
		p40[i] = y40;
		fprintf(out, "%.4f \t%.4f \t%.4f \t%.3f\n", y36, y38, y40, x[11][i]);
	...
	sgm36 = sqrt (( st36c / denom36 ) * ( sum36 / (n - 2 )));
	sgmm36 = sqrt (( sw36 / denom36 ) * ( sum36 / (n - 2)));
	sgm38 = sqrt (( st38c / denom38 ) * ( sum38 / (n -2 )));
	sgmm38 = sqrt (( sw38 / denom38 ) * ( sum38 / (n - 2)));
	sgm40 = sqrt (( st40c / denom40 ) * ( sum40 / (n - 2)));
	sgmm40 = sqrt (( sw40 / denom40 ) * ( sum40 / (n - 2)));
	fprintf(out,"\n");//datos de las rectas de la muestra
    ...
    discr = (0.250) * (( nier / atm4036 ) - 1 );
	sgdiscr = ( nier * sgatm4036 ) / (4.0 * atm4036 * atm4036 );
	...
	fprintf(out,"\n");//datos corregidos por blanco y discrimunacion
	...
	fprintf(out,"Atmos No %s  fecha atmos %s 40/36 atm %.1f err %.1f\n",
		AtmName, AtmDate, atm4036, sgatm4036);
	fclose(out);
	return 0;
}
