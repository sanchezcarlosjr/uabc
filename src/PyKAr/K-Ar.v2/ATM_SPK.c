...
#define nier 295.5
#define at3836 0.1869436
#define at4038 1580.6906
#define rlam1 1.0E+6
#define fonk 2.87E-02
#define sixca 0.390169
#define sevca 1536.1
#define k3839 0.01044
#define rlam 5.543E-10
#define senso 3.7E-8
#define fca 1.83
#define n36 0.0003
#define n37 0.0003
...
#define n39 0.0003
#define n40 0.0021
#define deca37 1.9804E-2
#define deca39 7.0286E-6
#define deca36 2.02E-6
...
void AtmosCalc(char *argIN, char *argOUT, float * m40_36,float * sd40_36)
{
int i, numero, nub;
FILE *in, *out;
	...
	fscanf (in, "%d", &nub);
	fprintf(out,"%s\n", nomb1);
fprintf(out, "36        37        38       39      40      VOLTS\n");
	for (i = 0; i<nub; i++){
		...
		a39 = f9 - (f8 + f10) / 2.0;
		a40 = f11 -  (f10 + f12) / 2.0;
fprintf(out, "%.4f\t%.4f\t%.4f\t%.4f\t%.4f\t%.3f\n", a36, a37, a38, a39, a40, f13);
		q36[i] = a36;
		q37[i] = a37;
		q38[i] = a38;
		q39[i] = a39;
		q40[i] = a40;
		sib36[i] = ((0.0012 * a36)*(0.0012 * a36) + (n36 * n36));
		sib37[i] = ((0.0012 * a37)*(0.0012 * a37) + (n36 * n36));
		sib38[i] = ((0.0012 * a38)*(0.0012 * a38) + (n38 * n38));
		sib39[i] = ((0.0012 * a39)*(0.0012 * a39) + (n36 * n36));
		sib40[i] = ((0.0012 * a40)*(0.0012 * a40) + (n40 * n40));
		v36 = 1.0 / sib36[i];
		v37 = 1.0 / sib37[i];
		v38 = 1.0 / sib38[i];
...
// calculo de picos al tiempo de equilibrio
	...
	blk37 = (bco37 < 0.0) ? 0.00001 : bco37;
	blk38 = (bco38 < 0.0) ? 0.00001 : bco38;
	blk39 = (bco39 < 0.0) ? 0.00001 : bco39;
fprintf(out,"\n");
fprintf(out,"36y0 %.4f 37y0 %.4f 38y0 %.4f 39y0 %.4f 40y0 %.4f\n", a036, a037, a038, a039, a040);
fprintf(out,"S36 %.2f S37 %.2f S38 %.2f S39 %.2f S40 %.2f\n", sub36, sub37, sub38, sub39, sub40);
fprintf(out,"m36 %f m37 %f m38 %f m39 %f m40 %f\n", l36, l37, l38, l39, l40);
fprintf(out,"err %f err %f err %f err %f err %f\n", sgl36, sgl37, sgl38, sgl39, sgl40);
fprintf(out,"\n");
fprintf(out,"Blanco calculado al tiempo de equilibrio y error -------- 40/36 = %.1f\n", blk40/blk36);
fprintf(out,"\n");
fprintf(out,"36 %f 37 %f 38 %f 39 %f 40 %f\n", blk36, blk37, blk38, blk39, blk40);
fprintf(out,"er %f er %f er %f er %f er %f\n", sgbl36, sgbl37, sgbl38, sgbl39, sgbl40);
...
fprintf(out, "%.4f\t%.4f\t%.4f \t%.4f\t%.4f\t%.3f\n", y36, y37, y38, y39, y40, d13);
		...
		p40[i] = y40;
		sig36[i] = ((0.0012 * y36)*(0.0012 * y36) + (n36 * n36));
		sig37[i] = ((0.0012 * y37)*(0.0012 * y37) + (n36 * n36));
		sig38[i] = ((0.0012 * y38)*(0.0012 * y38) + (n38 * n38));
		sig39[i] = ((0.0012 * y39)*(0.0012 * y39) + (n36 * n36));
		sig40[i] = ((0.0012 * y40)*(0.0012 * y40) + (n40 * n40));
		w36 = 1.0 / sig36[i];
		w37 = 1.0 / sig37[i];
		w38 = 1.0 / sig38[i];
	...
	denom37 = sw37 * st37c - st37 * st37;
	y037 = (s37 * st37c - st37 * sx37) / denom37;
	m37 = (sw37 * sx37 - s37 * st37) / denom37;
	denom38 = sw38 * st38c - st38 * st38;
	y038 = (s38 * st38c - st38 * sx38) / denom38;
	m38 = (sw38 * sx38 - s38 * st38) / denom38;
	denom39 = sw39 * st39c - st39 * st39;
	y039 = (s39 * st39c - st39 * sx39) / denom39;
	m39 = (sw39 * sx39 - s39 * st39) / denom39;
	...
	for(i=0; i<numero; i++){
		...
		sgm37 = sqrt (( st37c / denom37 ) * ( sum37 / (numero - 2 )));
		sgmm37 = sqrt (( sw37 / denom37 ) * ( sum37 / (numero - 2)));
		sgm38 = sqrt (( st38c / denom38 ) * ( sum38 / (numero -2 )));
		sgmm38 = sqrt (( sw38 / denom38 ) * ( sum38 / (numero - 2)));
		sgm39 = sqrt (( st39c / denom39 ) * ( sum39 / (numero - 2 )));
		sgmm39 = sqrt (( sw39 / denom39 ) * ( sum39 / (numero - 2)));
		sgm40 = sqrt (( st40c / denom40 ) * ( sum40 / (numero - 2)));
		sgmm40 = sqrt (( sw40 / denom40 ) * ( sum40 / (numero - 2)));
//		sqr36 = ( sgm36 / y036 ) *  ( sgm36 / y036 );
//		sqr37 = ( sgm37 / y037 ) *  ( sgm37 / y037 );
//		sqr38 = ( sgm38 / y038 ) *  ( sgm38 / y038 );
//		sqr39 = ( sgm39 / y039 ) *  ( sgm39 / y039 );
...
fprintf(out,"\n");
fprintf(out,"y036 %.4f y037 %.4f y038 %.4f y039 %.4f y040 %.4f\n", y036, y037, y038, y039, y040);
fprintf(out,"er36 %.4f er37 %.4f er38 %.4f er39 %.4f er40 %.4f\n", sgm36, sgm37, sgm38, sgm39, sgm40);
fprintf(out,"S36 %.2f S37 %.2f S38 %.2f S39 %.2f S40 %.2f\n", sum36, sum37, sum38, sum39, sum40);
fprintf(out,"m36 %f m37 %f m38 %f m39 %f m40 %f\n", m36, m37, m38, m39, m40);
fprintf(out,"err %f err %f err %f err %f err %f\n", sgmm36, sgmm37, sgmm38, sgmm39, sgmm40);
fprintf(out,"\n");
...
//correccion por blanco
		at36 = eq6 - blk36;
		sgat36 = sqrt ((sgbl36 * sgbl36) + (seq6 * seq6));
		at38 = eq8 - blk38;
		sgat38 = sqrt ((sgbl38 * sgbl38) + (seq8 * seq8));
		at40 = eq0 - blk40;
		sgat40 = sqrt ((sgbl40 * sgbl40) + (seq0 * seq0));
		matm4036 = (at40) / (at36);
		matm3638 = (at36) / (at38);
		sqat40 = ( sgat40 / at40 ) * ( sgat40 / at40 );
		sqat36 = ( sgat36 / at36 ) * ( sgat36 / at36 );
		sqat38 = ( sgat38 / at38 ) * ( sgat38 / at38 );
		sgmatm = ( matm4036 ) * sqrt ( sqat40 + sqat36 );
		sgatm = ( matm3638) * sqrt ( sqat36 + sqat38);
fprintf(out,"\n");
	fprintf(out,"Masa 36 %f Masa 38 %f Masa 40 %f\n", at36, at38, at40);
	fprintf(out,"err M36 %f err M38 %f err M40 %f\n", sgat36, sgat38, sgat40);
fprintf(out,"\n");
	fprintf(out,"Atmos 40/36 %.1f  error %.1f Atmos 36/38 %.1f error %.1f\n", matm4036, sgmatm, matm3638, sgatm);
	fclose(in);
	fclose(out);
	*m40_36= matm4036;
    *sd40_36= sgmatm;
...
void StdCalc(char *ArchStd,char *ArchDat,char *ArchRes)
{
int i, j = 0, numero, nub, inl;
FILE *in, *out;
	in = fopen(ArchDat, "rt");
	out = fopen (ArchRes, "wt");
	InitStdAtm();
	fscanf (in, "%[^\n]", nomb1);
fprintf(out,"%s\n", nomb1);
fprintf(out,"\n");
// nub es el numero de  fracciones
	fscanf (in, "%d", &nub);
	 for (j = 0; j<nub; j++){
		fscanf (in, "%d", &inl);
		fgetc(in);
...
fprintf(out,"\n");
	 fscanf (in, "%[^\n]", nomb3);
	 fscanf(in, "%s%[^\n]", &atlab, &date);
	 fscanf(in, "%f%f%f", &m4036, &sg4036, &peso);
	 fscanf(in, "%f%f%f%f%f", &dias, &age, &K, &tail1, &tail2);
	 fscanf (in, "%d", &numero);
fprintf(out,"%s\n", nomb3);
fprintf(out,"\n");
fprintf(out,"Atmos\t fecha\t  40/36atm\t  +/-\t  peso\n");
fprintf(out,"%s\t %s\t  %.1f\t  %.1f\t  %.4f\n", atlab, date, m4036, sg4036, peso);
fprintf(out,"Dias despues de irradiacion %.1f Edad std en Ma %.1f\n", dias, age);
fprintf(out,"\n");
	di = ( 0.250 ) * ( nier / m4036 - 1.0 );
	sddi = ( nier * sg4036 ) / ( 4.0 * m4036 * m4036 );
		sw36 = 0.0;
...
//* imprime los datos de la recta para cada fraccion *
fprintf(out,"\n");
fprintf(out,"S36 %f S37 %f S38 %f S39 %f S40 %f\n", sum36, sum37, sum38, sum39, sum40);
fprintf(out,"m36 %f m37 %f m38 %f m39 %f m40 %f\n", m36, m37, m38, m39, m40);
fprintf(out,"err %f err %f err %f err %f err %f\n", sgmm36, sgmm37, sgmm38, sgmm39, sgmm40);
/* calculo de los picos al tiempo de equilibrio *
	eq7 = m37 * 600.0 + y037;
	eqx6 = m36 * 600.0 + y036;
	eqxx6 = eqx6 - eq7 * tail1;
	eq6 = (eqxx6 < 0.0) ? 0.00001 : eqxx6;
	eq8 = m38 * 600.0 + y038;
	eq0 = m40 * 600.0 + y040;
	ax39 = m39 * 600.0 + y039;
	axx39 = ax39 - eq0 * tail2;
	eq9 = (axx39 < 0.0) ? 0.00001 : axx39;

	seq7 = sqrt( 360000.0 * sgmm37 * sgmm37 + sgm37 * sgm37);
	seq6 = sqrt( 360000.0 * sgmm36 * sgmm36 + sgm36 * sgm36 + seq7*seq7*tail1*tail1);
	seq8 = sqrt( 360000.0 * sgmm38 * sgmm38 + sgm38 * sgm38);
	seq0 = sqrt( 360000.0 * sgmm40 * sgmm40 + sgm40 * sgm40);
	seq9 = sqrt( 360000.0 * sgmm39 * sgmm39 + sgm39 * sgm39 + seq0*seq0*tail2*tail2);
fprintf(out,"\n");
  fprintf(out,"Datos calculados al tiempo de equilibrio y su error\n");
  fprintf(out,"36  %f 36t  %f 37  %f 38  %f 39  %f 39t  %f 40  %f\n", eqx6, eq6, eq7, eq8, ax39, eq9, eq0);
  fprintf(out,"36 err %f 37 err %f 38 err %f 39 err %f 40 err %f\n", seq6, seq7, seq8, seq9, seq0);
//* correccion por blanco linea *
	  ab36 = ((eq6 - blk36) < 0.0) ? 0.00001 : (eq6 -blk36);
	  ab37 = ((eq7 - blk37) < 0.0) ? 0.00001 : (eq7 -blk37);
	  ab38 = ((eq8 - blk38) < 0.0) ? 0.00001 : (eq8 -blk38);
...
//	  sgab36 = sqrt ((sgbl36 * sgbl36) + (seq6 * seq6));
	  sgab37 = sqrt ((sgbl37 * sgbl37) + (seq7 * seq7));
	  sgab38 = sqrt ((sgbl38 * sgbl38) + (seq8 * seq8));
	  sgab39 = sqrt ((sgbl39 * sgbl39) + (seq9 * seq9));
	  sgab40 = sqrt ((sgbl40 * sgbl40) + (seq0 * seq0));
//* correccion por discriminacion, decaimiento de 37Ar y 39Ar *
	  ar40 = ab40;
	  sd40 = sgab40;
	  ar37 = (exp(deca37 * dias)) * (ab37 /(1.0 + 3.0 * di));
	  sd37 = (exp(deca37 * dias)) * sgab37;
	  ar39 = (exp(deca39 * dias)) * (ab39 / (1.0 + di));
	  sd39 = (exp(deca39 * dias)) * sgab39;
	  ar39k = ar39 - (ar37 / sevca);
//* correccion por cloro *
	  az38 = ab38 / (1.0 + 2.0 * di);
	  az36 = ab36 / (1.0 + 4.0 * di);
	  ax38 = az38 - k3839 * ar39 - at3836 * (az36 - ar37 * 2.54E-04);
	  ar38 = (ax38<0.0) ? 0.00001 : ax38;
	  sd38 = sgab38;
	  ax36 = az36 - dias * deca36 * ar38;
	  ar36 = (ax36<0.0) ? 0.00001 : ax36;
	  sd36 = sqrt(sgbl36*sgbl36 + seq6*seq6);
	  azz36 = ax36 - ar37 * 2.54E-04;
fprintf(out,"\n");
  fprintf(out,"Picos corregidos por blanco y decaimiento\n");
  fprintf(out,"36  %f 37  %f 38  %f 39  %f 40  %f\n", az36, ar37, az38, ar39, ar40);
  fprintf(out,"err %f err %f err %f err %f err %f\n", sd36, sd37, sd38, sd39, sd40);
fprintf(out,"\n");
  fprintf(out,"Picos corregidos por calcio, cloro y potasio\n");
  fprintf(out,"36Atm  %f 38Cl  %f 39ArK  %f\n", azz36, ax38, ar39k);
	  r69 = ar36 / ar39;
	  r09 = ar40 / ar39;
	  r97 = ar39 / ar37;
	  r96 = ar39 / ar36;
//	  r79 = ar37 / ar39;
	  ...
	  f3 = ( 1 - f1) / ar39;
	  f5 = ( f1 * f1 * sevca ) / ar37;
	  f6 = (( f1 * f2 * sevca ) / ( ar37 )) + ((f1 * sixca ) / ( ar36));
	  f4 = nier * r69 * (f6 + (1 - f2) / ar39) - r09 * (f5 + f3);
	  f8 = f5 * r97 * ( - 1.0);
	  f9 = f8 * ( 1 - sixca / r69);
	  f7 = r09 * f8 * (- 1.0) + ( nier * r69 * f9 );
	  f11 = ( f1 * sixca ) / ( ar36 * r69 );
	  f10 = nier * ( r69 * f11 - (( 1.0 - f2) / ar39 ));
	  f13 = ((ar39 * f4) * ( - 1.0 )) / ( 1.0 + di );
	  f14 = (( - 3.0) * ar37 * f7 ) / ( 1.0 + ( 3.0 * di ));
	  f15 = (( - 4.0) * ar36 * f10 ) / ( 1.0 + ( 4.0 * di ));
	  f12 = f13 + f14 + f15 ;
	  r09k = ( 1 - f1 ) * r09 - (nier * ( 1.0 - f2) * r69 ) - fonk;
	  f16 = f3 * f3 * sd40 * sd40;
	  f17 = f4 * f4 * sd39 * sd39;
	  f18 = f7 * f7 * sd37 * sd37;
	  f19 = f10 * f10 * sd36 * sd36;
	  f20 = f12 * f12 * sddi * sddi;
	  e09k = sqrt (f16 + f17 + f18 + f19 + f20 );
	  jfac = ( 1 / r09k) * ( exp (rlam * age * rlam1) - 1);
	  sdjfac = jfac * ( e09k / r09k );
	  jota[j] = ( 1 / (sdjfac * sdjfac)) * jfac;
	  sdjota[j] = 1 / (sdjfac * sdjfac);
	  atm40 = nier * senso * ( 1 / peso) * ( ar36 - ( ar37 * ( sixca / sevca )));
	  rad40 = ( ar39 * senso * r09k ) / (( 1.0 - f1 ) * peso);
	  ca37k39 = ar37 / ( ar39 - ( ar37 / sevca ));
	  c39 = ( ar39 * senso ) / (( 1.0 - f1 ) * peso );
//	  c37 = ( ar37 * senso ) / peso ;
	  KFA = K / c39;
	  KFAC[j] = KFA;
	  r06 = (( 1.0 -f1) * ar40 - (fonk * ar39)) / ( ar36 * ( 1.0 - f2));
//*  cak = ca37k39 * fca;   *
fprintf(out,"\n");
fprintf(out,"39Ar %12.5E (40* /39K) %.4f err %.4f J %12.5E err %12.5E EDAD en Ma %.1f\n", c39, r09k, e09k, jfac, sdjfac, age);
fprintf(out,"(37Ca/39K) %12.5E 40Ar atm %12.5E 40Ar rad %12.5E\n", ca37k39, atm40, rad40);
fprintf(out,"F1 %12.5E F2 %12.5E (40/36)m %.2f K Factor %12.5E\n", f1, f2, r06, KFA);
fprintf(out,"\n");

}
//* procedimiento j-maker *

	for( j=0; j<nub; j++){
			wjo = wjo + jota[j];
			sdwjo = sdwjo + sdjota[j];
			KFACC = KFACC + KFAC[j];
}
		wjota = wjo / sdwjo;
		sdwjota = sqrt ( 1 / sdwjo );
		PKFAC = KFACC / nub;
fprintf(out,"Prom pesado J %12.5E err %12.5E Factor K %12.5E\n",
		wjota, sdwjota, PKFAC);
  ...
  //crear archivo std
  out= fopen(ArchStd,"wt");
//  out= fopen("cic12.std","wt");
  fprintf(out,"%.5e %12.5e %.5f %.5f %.5f",jfac,sdjfac,tail1,tail2,K);
  fclose(out);
