...
public:
    ...
    void __init() {
        ...
        int j = 0;
        for(int i = 0; i < ROW; i++){
            while (j < 50 && this->mat[i][j] != 2*this->mat[i][j-1]) {
                this->mat[i].push_back (rand() % 10 + 1);
                j++;
            }
        }
    ...
    int contains (int number) {
        std::vector<int> myvector (this->mat[0]);
        myvector.insert(myvector.end(), this->mat[1].begin(), this->mat[1].end());
        myvector.insert(myvector.end(), this->mat[2].begin(), this->mat[2].end());
        std::vector<int>::iterator it;
        it = find (myvector.begin(), myvector.end(), number);
        if (it != myvector.end())
            return 1;
        else
            return number;
...
template <class T>
class SetMatrix : public Matrix<T> {
    void temp() {
        set<int> first;
    }
};
